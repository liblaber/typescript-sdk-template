...
describe('test Pets', () => {
  ...
  describe('test showPetById', () => {
    test('test api call', () => {
      const scope = nock('http://petstore.swagger.io/v1')
        .get('/pets/accusantium')
        .reply(200, { data: {} });
      return sdk.pets.showPetById('accusantium').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('http://petstore.swagger.io/v1').get('/pets/eum').reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('http://petstore.swagger.io/v1')
        .get('/pets/quibusdam')
        .reply(404, { data: {} });
      return expect(async () => await sdk.pets.showPetById('quibusdam')).rejects.toThrow();
    });
  });
});
